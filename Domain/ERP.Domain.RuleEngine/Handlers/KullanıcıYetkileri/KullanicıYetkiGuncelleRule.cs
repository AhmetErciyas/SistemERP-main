...
{
    [Tag("KullanicıYetkiGuncelle")]
    public class KullanicıYetkiGuncelleRule : BaseCustomValidationRule<ERP.Data.Entities.kullaniciYetkileri, IKullaniciRepository, IYetkilerRepository>
    {
        protected override void Validate(IContext ctx, kullaniciYetkileri model, IKullaniciRepository kullaniciRepository, IYetkilerRepository yetkilerRepository)
        {
            ...
            KullanicıYetkiKontrol(ctx, model, kullaniciRepository).Wait();
        }

        private async Task KullanicıYetkiKontrol(IContext ctx, kullaniciYetkileri model, IKullaniciRepository kullaniciRepository)
        {
            if (model.id == null || model.id == 0)
                ctx.Insert(new Exception(".. seçmeniz zorunludur"));
            var grup = await kullaniciRepository.GetFirstOrDefaultAsync(q => q.id == model.id);
            if (grup == null)
                ctx.Insert(new Exception("Bu Sisteme Kayıtlı Değildir"));
        }
